...
@app.route('/motive')
#@app.route(base_url + '/motive')
def motive():
    return render_template('Our-Motive.html')
...
def results(filename):
    ...
    (class_ids, labels, boxes, confidences) = yolo_forward(net, LABELS, image, confidence_level=0.60)
    ...
    if len(class_ids) > 0:
        ...
        return render_template('Results.html', confidences=format_confidences, labels=labels,
            old_filename = filename, filename=new_filename)
    else:
        found = False
